...
RUN cp "$(swift build --package-path /build -c release --show-bin-path)/Run Channels" ./
...
CMD ["serve", "--env", "production", "--hostname", "0.0.0.0", "--port", "8182"]
